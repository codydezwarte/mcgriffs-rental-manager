...
function handleSignedContract_(d) {
  ...
  const signature = signatureBlob_(d.signatureDataUrl);
  const signatureBlock = signature
    ? '<div style="margin-top:20px"><strong>Customer Signature</strong><br><img src="cid:customerSignature" style="max-width:520px;width:100%;height:auto;border-bottom:1px solid #111;margin-top:8px"></div>'
    : '<p><strong>Customer Signature:</strong> Signature stored with the rental record.</p>';

  const html = layout_(
    "Signed Rental Agreement",
    `Hi ${safe_(d.customerName || "there")},
    <p>Thank you for renting from McGriff's. This email confirms your signed rental agreement.</p>
    <div style="background:#f3f4f6;padding:16px;border-radius:10px;margin:18px 0">
      <p><strong>Equipment:</strong> ${safe_(d.equipmentName)}</p>
      <p><strong>Checkout:</strong> ${format_(d.startAt)}</p>
      <p><strong>Due back:</strong> ${format_(d.dueAt)}</p>
      <p><strong>Rental amount:</strong> $${Number(d.rentalAmount || 0).toFixed(2)}</p>
      <p><strong>Deposit:</strong> $${Number(d.depositAmount || 0).toFixed(2)}</p>
    </div>
  ...
  const options = {
    to: d.email,
    subject: `Signed rental agreement - ${d.equipmentName}`,
    htmlBody: html,
    name: STORE.name,
    replyTo: STORE.email,
    body: strip_(html)
  };
  if (signature) options.inlineImages = { customerSignature: signature };
  MailApp.sendEmail(options);
  logEmail_("signedContract", d, "sent", options.subject);
}

function signatureBlob_(dataUrl) {
  if (!dataUrl || typeof dataUrl !== "string") return null;
  const match = dataUrl.match(/^data:(image\/(?:png|jpeg));base64,(.+)$/);
  if (!match) return null;
  const ext = match[1] === "image/jpeg" ? "jpg" : "png";
  return Utilities.newBlob(Utilities.base64Decode(match[2]), match[1], `customer-signature.${ext}`);
}
